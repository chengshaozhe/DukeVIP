...
class PushForwardNoise():
    ...
    def __call__(self, stepCount, playerGrid, bean1Grid, bean2Grid, action, goal, firstIntentionFlag):
        if goal != 0 and not firstIntentionFlag:
            firstIntentionFlag = True
            realAction = np.add(action, action)
            self.noiseStep = stepCount
        else:
            realAction = action

        realPlayerGrid = tuple(np.add(playerGrid, realAction))
        return realPlayerGrid, firstIntentionFlag, realAction, self.noiseStep
...
def chooseActionToShowPreference(playerGrid, bean1Grid, bean2Grid, actionSpace):
    ...
    actionGoalPresentIndex = [allActionGoal.index(1), allActionGoal.index(2)]

    actionIndex = random.choice(actionGoalPresentIndex)
    action = actionSpace[actionIndex]

    return action

class ForceToCommitNoise():
    def __init__(self, controller):
        self.actionSpace = controller.actionSpace
        self.noiseStep = None

    def __call__(self, stepCount, playerGrid, bean1Grid, bean2Grid, action, goalList, firstIntentionFlag):
        if sum(goalList) == 0 and not firstIntentionFlag:
            realAction = chooseActionToShowPreference(playerGrid, bean1Grid, bean2Grid, self.actionSpace)
            firstIntentionFlag = True
            self.noiseStep = stepCount
        else:
            realAction = action
