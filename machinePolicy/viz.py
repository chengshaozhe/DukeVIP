...
def draw_V(ax=None, V=None, S=()):
    ...
    ax.axes.matshow(data, cmap='Reds')
...
def draw_Q(axes=None, Q=None, S=(), A=()):
    axes_f = axes.flatten()
    for si, s in enumerate(S):
        ax = axes_f[si]
        pT = Q[si, ai, :]
...
def draw_policy_4d_softmax(ax=None, PI={}, V=None, S=(), A=()):
    ...
    for s in S:
        for a in A:
            draw_state_quiver_softmax(ax, s, PI[s], scale=1, color='w') #scale 1.5
...
def draw_state_quiver_softmax(ax, state=(), actions={}, scale=1.5, color='k', **kwargs):
    ...
    ax.quiver(X, Y, U, V, angles='xy', scale_units='xy', units='inches', scale=scale, color=color, **kwargs)
